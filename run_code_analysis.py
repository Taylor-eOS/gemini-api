...
def advance_model():
    ...
    if model_index >= len(settings.MODEL_NAMES):
        print("Quota exhausted on all models", file=sys.stderr, flush=True)
        sys.exit(1)

def is_unavailable(e):
    return isinstance(e, errors.APIError) and (e.code == 503 or "UNAVAILABLE" in str(e))
    ...
    while True:
        try:
            ...
            if is_unavailable(e):
                ...
                print(f"{current_model()} reported 503 unavailable during attempt {unavailable_streak} of {settings.MAX_UNAVAILABLE_RETRIES}", file=sys.stderr, flush=True)
                if unavailable_streak >= settings.MAX_UNAVAILABLE_RETRIES:
                    prev_model = current_model()
                    advance_model()
                    print(f"{prev_model} unavailable {unavailable_streak} times in a row, using {current_model()} instead", file=sys.stderr, flush=True)
                    attempt = 0
                    unavailable_streak = 0
                    continue
            else:
...
def check_error(e):
    if isinstance(e, errors.APIError):
        if e.code == 429 or "RESOURCE_EXHAUSTED" in str(e):
            prev_model = current_model()
            advance_model()
            print(f"{prev_model} exhausted, switching to {current_model()}", file=sys.stderr, flush=True)
            return True
        if e.code == 404 or "NOT_FOUND" in str(e):
            print("Resource not found", file=sys.stderr, flush=True)
            sys.exit(1)
    return False
